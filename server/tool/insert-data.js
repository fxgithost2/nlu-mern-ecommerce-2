...
mongoose
  .connect(
    'mongodb+srv://hphuc:hphuc@cluster0.w9vzvzi.mongodb.net/ecommerce?retryWrites=true&w=majority'
  )
  .then(() => {
...
const userData = [
  ...
  {
    _id: mongoose.Types.ObjectId('a00000000000000000000003'),
    firstName: 'Staff',
    lastName: 'Account',
    gender: 'other',
    email: 'staff2@nlu.com',
    phone: '0123446781',
    username: 'staff2',
    password: '123',
    role: 'staff',
  ...
  // role customer
  {
    _id: mongoose.Types.ObjectId('a00000000000000000000004'),
    firstName: 'Customer1',
    lastName: 'Account',
    gender: 'other',
    email: 'customer1@nlu.com',
    phone: '0123456780',
    username: 'customer1',
    password: '123',
    role: 'customer',
  ...
  {
    _id: mongoose.Types.ObjectId('a00000000000000000000005'),
    firstName: 'Customer2',
    lastName: 'Account',
    gender: 'other',
    email: 'customer2@nlu.com',
    phone: '0123456781',
    username: 'customer2',
    password: '123',
    role: 'customer',
...
function standardPolicy(str) {
  [' tại 3019 siêu thị toàn quốc (miễn phí tháng đầu)', ' - Tìm hiểu'].forEach(
    (item) => {
...
function splitProductName(name) {
  if (name.startsWith('Laptop')) {
    return name.replace('Laptop', '').trim();
  }

  name = StringUtils.removeMultiSpace(
    StringUtils.replaceAll(name, '16GB', ' ')
  );
  name = StringUtils.removeMultiSpace(
    StringUtils.replaceAll(name, '32GB', ' ')
  );
  name = StringUtils.removeMultiSpace(
    StringUtils.replaceAll(name, '64GB', ' ')
  );
  name = StringUtils.removeMultiSpace(
    StringUtils.replaceAll(name, '128GB', ' ')
  );
  name = StringUtils.removeMultiSpace(
    StringUtils.replaceAll(name, '256GB', ' ')
  );
  name = StringUtils.removeMultiSpace(
    StringUtils.replaceAll(name, '512GB', ' ')
  );
  name = StringUtils.removeMultiSpace(StringUtils.replaceAll(name, '1TB', ' '));

  if (name.startsWith('Máy tính bảng')) {
    return name.replace('Máy tính bảng', '').trim();
  }
  if (name.startsWith('Đồng hồ thông minh')) {
    return name.replace('Đồng hồ thông minh', '').trim();
  }
  if (name.startsWith('Điện thoại')) {
    return name.replace('Điện thoại', '').trim();
  }
...
async function insertProduct() {
  const filePath = process.cwd() + '\\product.all.json';
  ...
  let listProduct = [
    ...
    const catCompare = categoryMap[a.link]
      .toString()
      .localeCompare(categoryMap[b.link].toString());
    if (catCompare != 0) {
      return catCompare;
    }
    ...
    if (
      a.brandName.toLowerCase() === 'apple' &&
      b.brandName.toLowerCase() !== 'apple'
    ) {
      return -1;
    }

    if (
      a.brandName.toLowerCase() === 'samsung' &&
      b.brandName.toLowerCase() !== 'apple'
    ) {
      return 1;
    }

    return categoryMap[a.link]
      .toString()
      .localeCompare(categoryMap[b.link].toString());
  });
  ...
  for (let i = 0; i < listProduct.length; i++) {
    ...
    const brand = listBrand.filter(
      (item) => item.name.toLowerCase() === brandToFind
    )[0];
    ...
    if (element.youtubeVideoId) {
      product.video = element.youtubeVideoId;
    }
    if (element.productPolicies) {
      product.policies = element.productPolicies.map((item) =>
        standardPolicy(item)
      );
    }
    if (element.hightLightImgs) {
      product.hightLightPics = element.hightLightImgs;
    }
    if (element.specThumb) {
      product.specPicture = element.specThumb;
    }
    if (element.key) {
      product.crawlKey = element.key;
    }

    if (element?.productCatalogSub1Id) {
      product.categorySub1 = mongoose.Types.ObjectId(
        element.productCatalogSub1Id
      );
      if (element.productCatalogSub2Id) {
        product.categorySub2 = mongoose.Types.ObjectId(
          element.productCatalogSub2Id
        );
        if (element.productCatalogSub3Id) {
          product.categorySub3 = mongoose.Types.ObjectId(
            element.productCatalogSub3Id
          );
        }
      }
    }
    ...
    product.variants = element.variants.map((v) => ({
      variantName:
        v.variantName.trim() !== 'Hình sản phẩm' ? v.variantName.trim() : '',
      sku: v.sku,
      thumbnail: v.images[1].map((item) =>
        item.startsWith('//') ? `https:${item}` : item
      )[0],
      pictures: v.images[1]
        .slice(1)
        .map((item) => (item.startsWith('//') ? `https:${item}` : item)),
  ...
  for (let i = productQueue.length - 1; i >= 0; i--) {
    ...
    console.log(
      `[${++j}/${listProduct.length}] Insert product '${product.name}' success`
    );
  }
...
async function insertUsers() {
  ...
  for (let i = 0; i < userData.length; i++) {
    ...
    await newUser.save();
    console.log(
      `[${i + 1}/${userData.length}] Inserted user: ` + element.username
    );
  }
}
...
async function insertData() {
  ...
  //   console.log(
  //     `[${i + 1}/${categoryData.length}] Inserted category: ` + element.name
  //   );
  // }

  // console.log('--> Insert brand data');
  // let listBrand = [
  //   ...JSON.parse(fs.readFileSync(process.cwd() + '\\json\\brand.json')),
  // ];
  ...
  //   await newBrand.save();
  //   console.log(
  //     `[${i + 1}/${listBrand.length}] Inserted brand: ` + element.name
  //   );
  // }
